...
export default function SchedulePage() {
  ...
  const getDayCourses = (date: Date) => {
    return mockCourses.filter((course) => {
      ...
      return course.schedule.days.some(
        (day) =>
          day.toLowerCase() ===
          date.toLocaleDateString("default", { weekday: "long" }).toLowerCase()
      );
    });
  };
          ...
          <div className="p-8 sm:p-10">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
              <div className="min-w-0">
                <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-2 truncate">
                  Class Schedule
                </h1>
              ...
              <div className="space-y-8">
                <div className="flex items-center justify-between bg-white/80 backdrop-blur-sm p-4 rounded-xl shadow-sm">
                  ...
                  <span className="text-lg sm:text-xl font-semibold text-gray-900 bg-gradient-to-r from-blue-600 to-blue-700 bg-clip-text text-transparent truncate px-4">
                    {getWeekDates().start.toLocaleDateString()} -{" "}
                    {getWeekDates().end.toLocaleDateString()}
                  </span>
                  <button
                    onClick={() => {
                      const newDate = new Date(currentWeek);
                ...
                <div className="grid grid-cols-7 gap-2">
                  {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(
                    (day) => (
                      <div
                        key={day}
                        className="text-center font-semibold text-gray-700 py-3 bg-white/80 backdrop-blur-sm rounded-xl text-sm truncate"
                      >
                        {day}
                      </div>
                    )
