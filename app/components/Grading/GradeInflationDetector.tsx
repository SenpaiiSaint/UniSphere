...
export const GradeInflationDetector = ({ historicalData }: GradeInflationDetectorProps) => {
    ...
    if (trend.trend === 'inflation' && trend.significant) {
      return {
        ...
        suggestedCurve: data.map(d => ({
          year: d.year,
          adjustedGrade: Math.max(0, d.averageGrade - (trend.percentage / 100) * d.averageGrade)
        }))
      };
      ...
      return {
        ...
        suggestedCurve: data.map(d => ({
          year: d.year,
          adjustedGrade: Math.min(100, d.averageGrade + (trend.percentage / 100) * d.averageGrade)
        }))
      };
    }
    return null;
      ...
      {trend && (
        <div className="mb-4">
          <div className={`flex items-center gap-2 p-3 rounded-lg ${
            trend.trend === 'inflation' ? 'bg-yellow-50 text-yellow-800' : 'bg-blue-50 text-blue-800'
          }`}>
            ...
            <span>
              {trend.trend === 'inflation' ? 'Grade Inflation' : 'Grade Deflation'} Detected
            </span>
            <span className="font-medium">
              ({trend.percentage.toFixed(1)}% {trend.trend === 'inflation' ? 'increase' : 'decrease'})
            </span>
          </div>
        </div>
      )}
              ...
              <div className="space-y-2">
                {normalization.suggestedCurve.map((adjustment, index) => (
                  <div key={index} className="flex justify-between text-sm">
                    ...
                    <span>
                      {adjustment.adjustedGrade.toFixed(1)} (adjusted from {historicalData[index].averageGrade.toFixed(1)})
                    </span>
                  </div>
                ))}
              </div>
        ...
        <div className="mt-4">
          ...
          <div className="space-y-2">
            {historicalData.map((data, index) => (
              <div key={index} className="flex justify-between text-sm">
                <span>{data.year}:</span>
                <span>{data.averageGrade.toFixed(1)} (n={data.studentCount})</span>
              </div>
            ))}
          </div>
        </div>
