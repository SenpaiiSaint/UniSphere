...
export const EnrollmentManagement = () => {
  ...
  const [analytics, setAnalytics] =
    useState<EnrollmentAnalytics>(mockAnalytics);
  ...
  const loadAnalytics = async (courseId: string) => {
    try {
      setLoading(true);
      const analyticsData = await enrollmentService.getEnrollmentAnalytics(
        courseId
      );
  ...
  const handleAddToWaitlist = async (
    ...
    try {
      setLoading(true);
      const success = await enrollmentService.addToWaitlist(
        selectedCourse.id,
        studentId,
        priority
      );
  ...
  const handleRemoveFromWaitlist = async (studentId: string) => {
    ...
    try {
      setLoading(true);
      const success = await enrollmentService.removeFromWaitlist(
        selectedCourse.id,
        studentId
      );
      if (success) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Analytics Section */}
      {selectedCourse && <EnrollmentAnalyticsComponent analytics={analytics} />}
    </div>
  );
};
