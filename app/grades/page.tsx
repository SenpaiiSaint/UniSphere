...
export default function GradesPage() {
  ...
  const [sortField, setSortField] = useState<"studentName" | "grade">(
    "studentName"
  );
  ...
  const getFilteredGrades = () => {
    return mockGrades.filter(
      (grade) => !selectedCourse || grade.courseId === selectedCourse
    );
  };
  ...
  const passingRate = grades.length
    ? Math.round(
        (grades.filter((g) => g.grade >= 70).length / grades.length) * 100
      )
    : 0;
                                ...
                                <div className="space-y-4">
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {grade.assignments.map((assignment) => (
                                      <div
                                        key={`${grade.id}-${assignment.id}`}
                                        className="bg-white rounded-lg p-4 shadow-sm border border-gray-100"
                                      >
                                        <div className="flex items-center justify-between mb-2">
                                          <span
                                            className={`px-2 py-1 text-xs font-semibold rounded-full ${getAssignmentTypeColor(
                                              assignment.type
                                            )}`}
                                          >
                                            {assignment.type}
